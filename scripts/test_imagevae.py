...
架构说明 (修改后的2层下采样):
    ...
    - 1024: latent [B, 8, 128, 128]
    - 512: latent [B, 8, 64, 64]
...
def test_forward_pass_512(model):
    ...
    assert out.latent_dist.mean.shape == (1, 8, expected_latent_size, expected_latent_size), \
        f"潜在形状错误: {out.latent_dist.mean.shape}, 预期 (1, 8, {expected_latent_size}, {expected_latent_size})"
...
def test_encode_decode_512(model):
    ...
    assert z.shape == (1, 8, expected_latent_size, expected_latent_size), \
        f"潜在变量形状错误: {z.shape}"
    print("✓ 编码/解码分离测试通过")
...
def test_gradient_flow(model):
    ...
    # 验证latent维度 (256 / 8 = 32)
    print(f"Latent 形状: {out.latent_dist.mean.shape}  (预期: [1, 8, 32, 32])")
    assert out.latent_dist.mean.shape == (1, 8, 32, 32)
...
def test_dimension_flow(model):
    ...
    with torch.no_grad():
        ...
        print(f"编码器输出: {h.shape} (期望 [1, 16, 64, 64])")
        assert h.shape == (1, 16, 64, 64)

        # 追踪解码器维度
        print("\n--- 解码器 ---")
        z = h[:, :8, :, :]  # 只取均值部分 (latent_dim=8)
        print(f"潜在变量: {z.shape}")
...
def test_config_loading():
    ...
    config_path = Path("examples/wfivae2-image-1024.json")
...
def main():
    ...
    try:
        ...
        print("  - 512: latent [8, 64, 64]")
        print("  - 1024: latent [8, 128, 128]")
